          ...
          if (response.status == 'success') {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                myModal.hide();
                $(`.data_lp3[data-idinvoice = ${response.id_invoice}]`).find('.status_penagihan').empty();
                $(`.data_lp3[data-idinvoice = ${response.id_invoice}]`).find('.status_penagihan').append(
    ...
    $('.data_lp3').on("click", function() {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          $('.info-2column .total_penagihan').text(response.data.tagihan);
